...
Those `x` in turn are objects of `XPath` class. Essentially this is `Path` class where methods has been replaced by properties, so that they can be used in the *Command Expressions*. Note that this `XPath` has nothing to do with [this](https://en.wikipedia.org/wiki/XPath) one. Sample usage:
